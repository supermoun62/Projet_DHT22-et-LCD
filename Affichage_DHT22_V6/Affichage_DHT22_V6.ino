...
DHT dht(DHTPIN, DHTTYPE); // Instance librairie DHT
  ...
  float var1H = 1.2121; // En fonction de la courbe de tendance
  float var2H = 7.5056; // En fonction de la courbe de tendance
...
// Calcul H avec courbe de tendance
  valeurHcorrigee = (float)var1H*valeurH-var2H;     
if (isnan(H) || isnan(T))
  {  
    bitDefaut1 = HIGH;
...
else
  {
    ...
    bitDefaut2 = LOW;
    digitalWrite(pinLed13, LOW);    
// Appel Routine Affichage Hcorrigee  
    Affichage_Hcorrigee_LCD();
    delay(100);    
// Appel Routine Affichage Tcorrigee    
    Affichage_Tcorrigee_LCD();     
  }    
  //Serial.print(valeurT); Serial.print("---");Serial.print(valeurTcorrigee); 
  //Serial.println();
  //Serial.print(valeurH); Serial.print("---");Serial.print(valeurHcorrigee); 
  //Serial.println();
...
void Affichage_Hcorrigee_LCD()
{
  ...
  lcd.print("Humidite:");
  lcd.setCursor(2,1);
  lcd.print(valeurHcorrigee,1);// 1 = Une decimale 
  lcd.setCursor(6,1);
  lcd.print("%");    
}// FIN VOID 
//--------------------------------------------
...
{
  lcd.setCursor(11,0);// // colonne, ligne 
  lcd.print("Temp:");  
  lcd.setCursor(11,1);  
  lcd.print(valeurTcorrigee,1);// 1 = Une decimale
  lcd.print(valeurT,1);
  lcd.setCursor(15,1);
